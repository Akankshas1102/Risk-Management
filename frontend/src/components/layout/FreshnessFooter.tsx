...
export function FreshnessFooter({ sidebarWidth }: Props) {
  ...
  return (
    <footer
      className="fixed bottom-0 right-0 z-30 flex h-8 items-center gap-4 border-t border-border bg-card/80 backdrop-blur px-6 text-xs text-muted-foreground transition-all duration-200"
      style={{ left: sidebarWidth }}
    >
      {pipelineAt && (
        <span className="flex items-center gap-1.5">
          <span
            className={cn('h-1.5 w-1.5 rounded-full', statusOk ? 'bg-success' : 'bg-warning')}
          />
